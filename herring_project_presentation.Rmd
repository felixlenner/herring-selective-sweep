...
annotate_positions <- function(input_df, annotation_df) {
  ...
  output_df <- inner_join(input_df, untangled_annotations, by = "POS") # add annotations to input dataframe 
...
# biomaRt
.tiny[
```{r biomart, comment='#', message=F, warning=F}
...
zfin_ids <- annotated_positions %>%            # Take our annotated positions
  ...
  summarize(mean.pos = mean(POS),              # Summarize like code above
            mean.freq = mean(FREQ), Gene_ID, ID) %>% 
  ...
  na.omit() %>%                               # keep genes with zfin_id's
