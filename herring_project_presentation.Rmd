...
- The Baltic Sea was formed approximately 15,000 years ago, during its creation Atlantic herring migrated into the new habitat causing a bottleneck of their gene pool and forming a population that would become the subspecies Baltic herring.
...
```
---
# Balsfjord are outliers (nice to keep?)
```{r, list-outliers}
# Filter the PCA datas for the atlantic outliers 
eigenvec %>% filter(PC1 > 0 & Area == "Atlantic") %>% select(ID1)
```
(skip this part?)
```{bash, eval=F, echo=T}

# Extract samples, produce new vcf files - to appendix!

# Get sample names
bcftools query -l chr4.recode.vcf | 
  grep "Baltic" > baltic_samples.txt

bcftools query -l chr4.recode.vcf | 
  grep "Atlantic" | grep -v "Balsfjord" > atlantic_samples.txt
...
Calculate the delta allele frequency
```{r tidy=FALSE, comment='#', delta-allele-frequencies}
...
---
# Plot alleles in R
```{r, message=FALSE, warning=FALSE,fig.height=3, dev='png'}
