...
annotated_positions %>% group_by(Gene_ID) %>%
  ...
  summarize(mean.POS = as.integer(mean(POS)), mean.FREQ = mean(FREQ)) %>% print(n = 5)
```
]
...
# biomaRt
.tiny[
```{r biomart, eval=F, comment='#', message=F, warning=F, dependson="setup"}
...
ensembl = useMart("ensembl",dataset="charengus_gene_ensembl",host="https://useast.ensembl.org/")
...
combined_data <- rbind(atlantic_allele_freq, 
                       ...
                       delta_het_freq)
```
# Summary (change plots? two slides?)
.pull-left[
...
combined_data %>%
  ...
  geom_point(data = annotated_gene_names, aes(x = mean.pos/1e6, y = mean.freq, color = "red"), alpha = 1, size = .3) +
