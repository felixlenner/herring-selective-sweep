...
Locate the peaks and extents of selective sweeps occurring the chromosome 4 of the baltic herring genome
...
```
---
# Prepare PCA/Read files (double-skip?)
...
```
---
# PCA plot (keep plot, skip code? add more code?)
...
---
# Missingness plot (would be good to keep I think)
...
```
---
# Show here how the plots looked like after removing bad SNPs - only delta?
```{r, message=FALSE, warning=FALSE,fig.width=14,fig.height=5, dev='png'}
combined_allele_freq %>%
...
---
# Annotation code stuff (maybe not super relevant?)
.tiny[
```{bash bcftools-ann, eval=F}
...
---
# More Annotation code (keep only filtering part?)
