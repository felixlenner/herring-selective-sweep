...
- By determining the presence of selective sweeps in the chromosome, it should be possible to discover the genes that mutated (and fixed in the population) in order to allow the herring to adapt to their new environment.
...
- Had to filter chromosome 4 from the file in order to have a workable size file. 

- Used PCA analysis to study the provided data to understand population structure

- Filter unwanted populations and samples from the study
...
- Determine selective sweep peak regions
...
# Heterozygosity (should we just skip het completely? just show/explain missing?)
...
# After filtering (show only delta?)
...
# Should we put a zoomed in plot here?
...
# Summary (change plots?)

```{r plot-delta-gene-names, eval=T, echo=FALSE, warnings=F, message=F}
library(ggrepel)
...
combined_data %>%
  ...
  geom_point(data = annotated_gene_names, aes(x = mean.pos/1e6, y = mean.freq, color = "annotated genes with gene names"), alpha = 1, size = .3) + theme_minimal() + 
 # geom_text_repel(data = annotated_gene_names, 
  #                aes(x = mean.pos/1e6, y = ifelse(mean.freq>0.6,mean.freq, NA), label = external_gene_name), 
   #               max.overlaps = Inf, 
